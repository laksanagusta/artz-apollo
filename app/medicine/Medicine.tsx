...
const Medicine = () => {
  ...
  const getMedicine = async (
    ...
    const { data } = await _medicineService.getMedicine(
      searchParams,
      page,
      limit
    );
  ...
  const onLoadMedicine = async (): Promise<void> => {
    await getMedicine(initialValues.searchParams, initialValues.limit, 0);
  };

  const handlePageClick = async ({ selected }: any) => {
    await getMedicine(searchParams, limit, selected);
  };

  async function onSubmit(value: any, { setSubmitting }: any): Promise<void> {
    await getMedicine(value.searchParams, parseInt(value.limit), 0);
